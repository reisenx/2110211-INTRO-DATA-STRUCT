...
class priority_queue
{
  ...
  public:
    ...
    void erase(const T& v) {
      // Search index of item v
      // If found idx < mSize, but if not found idx = mSize
      int idx = 0;
      for(idx = 0; idx < mSize; idx++) { if(mData[idx] == v) break; }
      // If found (idx < mSize), fix the binary heap
      if(idx < mSize)
      {
        // Replace mData[idx] (deleted node) to the rightmost child of the deepest level
        mData[idx] = mData[mSize - 1];
        // Setup new mSize
        mSize--;
        // Using fixDown() to find a appropriate value to replace mData[idx]
        fixDown(idx);
        // Using fixUp() to make sure we maintain the binary heap rule
        fixUp(idx);
      }
    }
