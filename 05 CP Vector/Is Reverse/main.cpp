...
class vector
{
  ...
  public:
    ...
    // Edit only in this function
    bool isReverse(const vector<T> &other) const {
      // Case 1: The size of both CP::vector is not equal
      if (mSize != other.size()) return false;
      // Case 2: The size of both CP::vector is equal
      for(size_t i = 0; i < mSize; i++)
      {
        // Check if a member at the front and the back (when reversed) is equal
